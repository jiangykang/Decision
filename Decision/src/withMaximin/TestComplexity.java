...
//								System.out.print(list2[0]+","+list2[1]+" ");
							...
							if(exist) {
								...
								if(ifGenxin) {
									...
									List<int[]> list_update = new ArrayList<>(list.get(v));
//									for (int[] ks : list_update) {
//										System.out.println(ks[0]+","+ks[1]);
//									}
									list_update.add(add);									
										for(int x=0;x<list_update.size();x++) {
											int tail = ((int[])list_update.get(x))[1];
											for(int y=0;y<list_update.size();y++) {
												int head = ((int[])list_update.get(y))[0];
												if(tail==head) {
													int a = ((int[])list_update.get(x))[0];
													int b = ((int[])list_update.get(y))[1];
													int[] c = {a,b};
													boolean flag = true;
													for (int l = 0; l < list_update.size(); l++) {
														if(Arrays.toString(list_update.get(l)).equals(Arrays.toString(c))) {
															flag = false;
															break;
										...
										List<Integer> upOn = cal_partialOrderUpon(list_update, i-1);
										for(int k=1;k<=allCandidate.length;k++) {
							...
							if(exist) {
								...
								if(ifGenxin) {
									...
									list_update.add(add);									
										...
										List<Integer> down = cal_partialOrderDown(list_update, i-1);
										for(int k=1;k<=allCandidate.length;k++) {
